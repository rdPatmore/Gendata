...
if salt:
    ...
    s = s0 + s1
    s = np.full(state.gridx.shape, 34.4) # uniform pforce
...
if shice_0:
    ...
    #h1 = - 2 
    #dhdx = (h1 - h0) / (xdim -3)
    #shice_topo =  h0 - (dhdx) + (mesh * dhdx)
    #shice_topo[:,:2]  = h0 
    #shice_topo[:,-2:] =  h1
    shice_topo[:,:10] = -2
    print (shice_topo.shape)
...
if shice:
    ...
    hFacC = state.readBin('../SHELF_039_hFacC.data', x=int(xdim), y=int(ydim),
                                                     z=int(zdim))
